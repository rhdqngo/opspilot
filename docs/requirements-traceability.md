...
| FR-017 | Partial | Each logical evidence tool emits the fixed privacy-safe `ToolCallAuditEvent` schema with scope, timing, result, truncation/cache, and safe error fields. The iterative candidate propagates the stored Runtime `run_id` through Task and executor and passes fixed-schema local tests; managed Preview v2 verification is pending. |
...
| FR-022 | Partial | The enabled registration and English/privacy/negative-boundary Preview flows are verified. The iterative candidate addresses healthy final scheduling, Korean incident-ID handling, localized output, and visible default-DEV assumptions locally; the complete Preview v2 matrix is pending. |
